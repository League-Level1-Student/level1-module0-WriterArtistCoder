...
/* CowSim v1.0.0 TODO Update when version is changed
 * The code in which
 * 
 * try {
 * 		X_____ = new ImageIcon(ImageIO.read(new MyFirstSwingGUI().getClass().getResourceAsStream(Y_____)));
 * } catch (IOException e) {
 * 		X_____ = null;
 * }
 * 
 * is used is from The League of Amazing Programmers (jointheleague.org).
 * */
...
public class CowSim {
	...
	public void updateGraphics(CowSim simulator, JFrame ui) {
		...
		while (true) {
			...
			ui13.setVisible(billTime >= billTimestep); // Show "Pay bills" button if it's time to
